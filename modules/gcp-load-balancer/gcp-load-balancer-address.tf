#
# GCP Load Balancer Address allocations
#

resource "google_compute_address" "gclb" {
  #
  # Regional Compute Addresses
  #
  for_each = {
    for address in local.gcp_lb_compute_address : address.resource_index => address
...
locals {
  ...
  google_compute_address = merge(
    google_compute_address.gclb,
    google_compute_global_address.gclb,
  )
}
