...
locals {
  google_forwarding_rule_target = merge(
    # Proxies
...
resource "google_compute_forwarding_rule" "lz" {
  #
  # GCPLB Forwarding Rules (Regional)
  #
  for_each = {
    for rule in local.gcp_lb_forwarding_rule : rule.resource_index => rule
    if rule.region != null
  }
  ...
  backend_service         = each.value.backend_service == null ? null : lookup(local.google_compute_backend_service, each.value.backend_service, null) == null ? each.value.backend_service : local.google_compute_backend_service[each.value.backend_service].self_link
  load_balancing_scheme   = each.value.load_balancing_scheme
  network                 = each.value.network
  port_range              = each.value.port_range
  ports                   = each.value.ports
  subnetwork              = each.value.subnetwork
  target                  = each.value.target == null ? null : lookup(local.google_forwarding_rule_target, each.value.target, null) == null ? each.value.target : local.google_forwarding_rule_target[each.value.target].self_link
...
resource "google_compute_global_forwarding_rule" "lz" {
  #
  # GCPLB Forwarding Rules (Global)
  #
  for_each = {
    for rule in local.gcp_lb_forwarding_rule : rule.resource_index => rule
    if rule.region == null
