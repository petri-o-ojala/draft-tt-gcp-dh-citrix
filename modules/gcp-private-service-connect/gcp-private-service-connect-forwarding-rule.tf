...
locals {
  ...
  google_compute_forwarding_rule = merge(
    ...
    local.gcp_compute_forwarding_rule
  )
}
...
resource "google_compute_forwarding_rule" "vpc" {
  ...
  #
  for_each = {
    for rule in local.gcp_psc_forwarding_rule : rule.resource_index => rule
    if rule.region != null
  }
  ...
  target                  = each.value.target == null ? null : lookup(local.gcp_forwarding_rule_target, each.value.target, each.value.target)
